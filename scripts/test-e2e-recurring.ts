// End-to-end test of authorize_recurring_delegate + cancel_recurring_delegate
// under real passkey auth: fresh vault, fund wallet_authority, authorize a
// recurring schedule (confirming the on-chain SPL Approve CPI actually
// landed), then cancel it (confirming the account closes and rent returns).
...
async function main() {
  ...
  await program.methods
    .initSavingsGoal("E2E Recurring Test Goal", new anchor.BN(1000 * 10 ** USDC_DECIMALS), { usdc: {} }, new anchor.BN(oneYearFromNow), DEVNET_USDC_MINT)
  ...
  const authPlaceholder = await program.methods
    ...
    .instruction();
  const authProof = await signForInstruction(authPlaceholder, 1 + 8 + 1 + 8, devicePriv, primaryDevicePubkey);
  const realAuthIx = await program.methods
    .authorizeRecurringDelegate(DEVICE_ROLE_PRIMARY, new anchor.BN(amountPerPeriod), { weekly: {} }, new anchor.BN(nextRunAt), authProof.authenticatorData, authProof.clientDataJson)
    .accounts({
      payer: mainWallet.publicKey,
      instructionsSysvar: SYSVAR_INSTRUCTIONS_PUBKEY,
      vault: vaultPda,
  ...
  console.log("Sending real authorize_recurring_delegate transaction...");
  try {
    const sig = await provider.sendAndConfirm(new Transaction().add(authProof.precompileIx).add(realAuthIx));
    console.log("SUCCESS. signature:", sig);
    const tokenAccountInfo = await getAccount(connection, walletAuthorityUsdcAta);
    console.log("  source_token_account delegate:", tokenAccountInfo.delegate?.toBase58());
    console.log("  delegated amount:", tokenAccountInfo.delegatedAmount.toString(), "(expected", amountPerPeriod * 100, ")");
  } catch (err: any) {
    console.log("FAILED (authorize):", err.message ?? err);
    if (err.logs) err.logs.forEach((l: string) => console.log("  " + l));
    return;
  }

  // --- cancel_recurring_delegate ---
  const cancelPlaceholder = await program.methods
    ...
    .instruction();
  const cancelProof = await signForInstruction(cancelPlaceholder, 1, devicePriv, primaryDevicePubkey);
  const realCancelIx = await program.methods
    .cancelRecurringDelegate(DEVICE_ROLE_PRIMARY, cancelProof.authenticatorData, cancelProof.clientDataJson)
    .accounts({
      payer: mainWallet.publicKey,
      instructionsSysvar: SYSVAR_INSTRUCTIONS_PUBKEY,
      vault: vaultPda,
  ...
  console.log("Sending real cancel_recurring_delegate transaction...");
  try {
    const sig = await provider.sendAndConfirm(new Transaction().add(cancelProof.precompileIx).add(realCancelIx));
    console.log("SUCCESS. signature:", sig);
    const closedAccountInfo = await connection.getAccountInfo(delegatePda);
    console.log("  delegate account after cancel:", closedAccountInfo === null ? "CLOSED (correct)" : "STILL EXISTS (unexpected)");
  } catch (err: any) {
    console.log("FAILED (cancel):", err.message ?? err);
    if (err.logs) err.logs.forEach((l: string) => console.log("  " + l));
  }
}
