...
Family Cookie is a savings-goal wallet. You connect a wallet, create a named
goal with a target amount and a deadline, fund a personal holding account
from your wallet, then move money into the goal itself with a device
passkey instead of a routine wallet signature. There is no swap, no price
feed, no order book, and no trading logic anywhere in the program.

## Why this is different from most other submissions

Most Cookie Chain bounty entries in this round are swap terminals, trading
dashboards, or other degen-facing tools built around liquidity and price
action. Family Cookie deliberately isn't one of those. It's built around a
single mechanism: real money moving toward a real goal, gated by a device
passkey (secp256r1, verified on-chain via WebAuthn, not a seed phrase or a
routine wallet click). The differentiation isn't cosmetic; the on-chain
program has zero trading-related instructions in it.

## Live deployment

- App: `https://family-cookie-tjscode.netlify.app` (target hosting URL;
  confirm this is actually live before relying on it, see the open items
  below)
- Cookie Chain program ID: not yet available. The program has been ported
  and the config is in place (`Anchor.toml`, `frontend/js/config.js`), but
  the actual `anchor deploy` to Cookie Chain has not been confirmed
  completed as of this write-up. This line will be filled in once that
  deploy is confirmed.
- Solana devnet program ID: `6apQZUxBwpBbFNiBpYzGeMBQJ8328xrrsQ3aRcQhM2Av`

## Contribution history

The app includes a read-only view of a goal's real on-chain contribution
history (timestamps and amounts of actual transfers into the goal's token
account), read directly from the chain's own RPC. This is not a CookieScan
integration; CookieScan (cookiescan.io) has no discoverable public API as
of this build, so that specific ecosystem-tool integration was skipped
rather than built against undocumented endpoints under time pressure.
